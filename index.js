...
function findEvenNumber(numbers) {
  ...
  try {
    for (let i = 0; i < numbers.length; i++) {
      if (numbers[i] % 2 === 0) { // Шукаємо перше число, що ділиться на 2 без остачі
        evenNumber = numbers[i]; // записуємо в нашу змінну
        return evenNumber; 
      }
      }
    if (!evenNumber) {
      throw new Error('У масиві немає чисел, що діляться на 2 без остачі!') // Якщо такого числа немає, кидаємо помилку
    } 
  } catch (error) {
    return error.toString();
    }
